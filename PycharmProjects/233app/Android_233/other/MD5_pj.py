    ...
    if len(passwd) != 32:
        ...
        try:
            ...
            def md5crack():
                ...
                try:
                    ...
                    if match:
                        print '[-] site: %s\t\t\tPassword: %s' % (site, match.group(3))
                    else:
                        print '[-] site: %s\t\t\tPassword: Not found' % site
                except urllib2.URLError:
                    print '[+] site: %s \t\t\t[+] Error seems to be down' % site
            ...
            def rednoize():
                ...
                try:
                    ...
                    if not len(data):
                        ...
                        print '[-] site: %s\t\t\tPassword: %s' % (site, data)
                except urllib2.URLError:
                    print '[+] site: %s \t\t\t[+] Error: seems to be down' % site
            ...
            def hashkiller():
                ...
                try:
                    ...
                    if match:
                        ...
                        print '[-] site: %s\t\t\tPassword: Not found' % site.replace('http://', '')
                except urllib2.URLError:
                    print '[+] site: %s \t\t\t[+] Error: seems to be down' % site
        ...
        Gecko/20100316 AskTbSPC2/3.9.1.14019 Firefox/3.6.2')
          ...
          (?P<hash>[^>]+)</td></tr></tbody></table>', data)
                    if match:
                        print '[-] site: %s\t\t\tPassword: %s' % (site, match.group(hash))
                    else:
                        print '[-] site: %s\t\t\tPassword: Not found' % site
                except urllib2.URLError:
                    print '[+] site: %s \t\t\t[+] Error: seems to be down' % site
            ...
            def cloudcracker():
                ...
                try:
                    ...
                    if match:
                        ...
                        print '[-] site: %s\t\t\tPassword: Not found\n' % site
                except urllib2.URLError:
                    print '[+] site: %s \t\t\t[+] Error: seems to be down' % site
